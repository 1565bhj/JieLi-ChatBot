...
AT(.volatile_ram_code)
___interrupt
static void exception_analyze_sram(void) __attribute__((noinline))
{
#if 0
...
void exception_analyze(int *sp) __attribute__((noinline))
{
    int len = 0;

    log_output_release_deadlock();
#if (!defined PRODUCTION_ALL_TEST_ENABLE)
