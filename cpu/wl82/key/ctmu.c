...
//ctmu才支持不可屏蔽中断方式
#define SEC_CODE    SEC_USED(.volatile_ram_code)
static sCTMU_KEY_VAR ctm_key_value SEC_USED(.sram);
static u8 port_index_mapping_talbe[CTMU_KEY_CH_MAX] SEC_USED(.sram);
#else
#define SEC_CODE
...
static void ctm_irq(sCTMU_KEY_VAR *ctm_key_var, u32 ctm_res, u8 ch) SEC_CODE {
...
___interrupt
static void ctmu_isr_handle(void) SEC_CODE {
    u32 res = CTMU_RES;
    u8 ch = CTMU_CON1 & 0xf;
...
#ifndef CONFIG_IPMASK_ENABLE
    if (num >= 3) {
        ASSERT(0, "CTMU_TOUCH_KEY no surppoort >= 3 channel");
    }
#endif
    if (num >= 6) {
        CTMU_CON0 |= (GATE_SOURCE_PRE_DIV8192 << 5); //0.34ms
    } else if (num >= 3) {
        CTMU_CON0 |= (GATE_SOURCE_PRE_DIV16384 << 5); //0.68ms
    } else {
...
int ctmu_init(void *_data)
{
    ...
    request_irq(IRQ_CTM_IDX, 1, ctmu_isr_handle, 1);
