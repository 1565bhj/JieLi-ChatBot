...
void mbedtls_net_set_timeout(mbedtls_net_context *ctx, int send_to_ms, int recv_to_ms)
{
    ...
    //add by dhx
    if (ctx->send_to_ms && ctx->hdl) {
        sock_set_send_timeout((void *)ctx->hdl, ctx->send_to_ms);
    }
    if (ctx->recv_to_ms && ctx->hdl) {
        sock_set_recv_timeout((void *)ctx->hdl, ctx->recv_to_ms);
    }
}

void mbedtls_net_set_connect_timeout(mbedtls_net_context *ctx, int connect_to_ms)
{
    ctx->connect_to_ms = connect_to_ms;
    if (ctx->connect_to_ms && ctx->hdl) {
