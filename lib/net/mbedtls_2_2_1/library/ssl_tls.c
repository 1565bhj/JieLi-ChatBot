...
#ifdef MBEDTLS_SSL_MEM_SMALL
            static char mbedtls_buf[MBEDTLS_SSL_BUFFER_LEN] ALIGNED(4);
            ssl->compress_buf = mbedtls_buf;
#else
...
/*
 * Setup an SSL in out buf
 */
int mbedtls_ssl_in_out_buf_static_set(mbedtls_ssl_context *ssl, char *in_buf, int in_size, char *out_buf, int out_size)
{
    if (in_size < MBEDTLS_SSL_BUFFER_LEN || out_size < MBEDTLS_SSL_BUFFER_LEN) {
        MBEDTLS_SSL_DEBUG_MSG(1, ("in_size < %d , out_size < %d", in_size, out_size));
        return MBEDTLS_ERR_SSL_ALLOC_FAILED;
    }
    ssl->in_buf = in_buf;
    ssl->out_buf = out_buf;
                      ...
                      const mbedtls_ssl_config *conf)
{
     ...
     */
#ifdef MBEDTLS_SSL_MEM_SMALL
//    static char static_in_buf[MBEDTLS_SSL_BUFFER_LEN];
//    static char static_out_buf[MBEDTLS_SSL_BUFFER_LEN];
//    memset(static_in_buf, 0, sizeof(static_in_buf));
//    memset(static_out_buf, 0, sizeof(static_out_buf));
//    ssl->in_buf = static_in_buf;
//    ssl->out_buf = static_out_buf;

    if (!ssl->in_buf) {
        ssl->in_buf = mbedtls_calloc(1, len + 1);
        if (!ssl->in_buf) {
            return (MBEDTLS_ERR_SSL_ALLOC_FAILED);
        }
        ssl->in_buf[MBEDTLS_SSL_BUFFER_LEN + 1] = (unsigned char)MBEDTLS_SSL_BUF_MAGIC;
    }
    if (!ssl->out_buf) {
        ssl->out_buf = mbedtls_calloc(1, len + 1);
        if (!ssl->out_buf) {
            if (ssl->in_buf && ssl->in_buf[MBEDTLS_SSL_BUFFER_LEN + 1] == (unsigned char)MBEDTLS_SSL_BUF_MAGIC) {
                mbedtls_free(ssl->in_buf);
                ssl->in_buf = NULL;
            }
            return (MBEDTLS_ERR_SSL_ALLOC_FAILED);
        }
        ssl->out_buf[MBEDTLS_SSL_BUFFER_LEN + 1] = (unsigned char)MBEDTLS_SSL_BUF_MAGIC;
    }
#else
...
void mbedtls_ssl_free(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->out_buf != NULL) {
#ifdef MBEDTLS_SSL_MEM_SMALL
        if (ssl->out_buf && ssl->out_buf[MBEDTLS_SSL_BUFFER_LEN + 1] == (unsigned char)MBEDTLS_SSL_BUF_MAGIC) {
            mbedtls_zeroize(ssl->out_buf, MBEDTLS_SSL_BUFFER_LEN + 1);
...
#ifdef MBEDTLS_SSL_MEM_SMALL
        if (ssl->in_buf && ssl->in_buf[MBEDTLS_SSL_BUFFER_LEN + 1] == (unsigned char)MBEDTLS_SSL_BUF_MAGIC) {
            mbedtls_zeroize(ssl->in_buf, MBEDTLS_SSL_BUFFER_LEN + 1);
            mbedtls_free(ssl->in_buf);
        } else {
