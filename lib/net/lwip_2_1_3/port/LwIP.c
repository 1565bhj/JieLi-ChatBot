...
static const dns_redirect_entry_t dns_redirects[] = {
    {"captive.apple.com"},
...
int __attribute__((weak)) dns_set_network_detect_redirect(void)
{
    return 0;
}
...
#endif
    if (dns_set_network_detect_redirect()) {
        for (int i = 0; i < ARRAY_SIZE(dns_redirects); i++) {
            dns_redirect_entry_t *dns = &dns_redirects[i];
            if (dns->domain) {
                printf("dns_local_addhost %s, ip : %s\n", dns->domain, inet_ntoa(netif->ip_addr));
                if (dns_local_addhost(dns->domain, &netif->ip_addr) != ERR_OK) {
                    printf("dns_local_addhost err : %s\n", dns->domain);
                }
            }
        }
    }
