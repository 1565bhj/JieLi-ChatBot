...
void lv_port_fs_init(void)
{
    ...
    /*Set up fields...*/
    fs_drv.letter = 's';//适配SD卡路径的参数，第一个SD卡根目录的路径必须是s开头
    ...
    /*Set up fields...*/
    fs_drv_m.letter = 'm';//适配flash路径的参数，第一个flash卡根目录的路径必须是m开头
...
#if 0//def CONFIG_JLFAT_ENABLE
    char _path[64] = {0};
    long_file_name_encode(path, (unsigned char *)_path, sizeof(_path));//这个函数有问题
    file = fopen(_path, "r");
    if (file == NULL) {
