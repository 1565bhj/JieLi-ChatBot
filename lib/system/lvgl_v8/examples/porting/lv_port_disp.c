...
void __attribute__((weak)) lvgl_disp_rounder_cb(lv_disp_drv_t *disp_drv, lv_area_t *area)
{
}
void __attribute__((weak)) lvgl_disp_init(void)
{
    //user_lcd_init();
}
void __attribute__((weak)) lvgl_disp_get_width_hight(int *width, int *hight)
{
    if (width && hight) {
        *width = 240;
        *hight = 320;
...
void __attribute__((weak)) lvgl_disp_flush(int xs, int xe, int ys, int ye, unsigned char *img)
{
...
void lv_port_disp_init(void)
{
    ...
    lvgl_disp_get_width_hight(&disp_width, &disp_hight);//获取应用层的屏幕分辨率
#if 0
    /* Example for 1) */
...
//    static lv_color_t buf_3_2[MY_DISP_HOR_RES * MY_DISP_VER_RES] __attribute__((aligned(4)));            /*Another screen sized buffer*/
    lv_color_t *buf_3_1 = lv_mem_alloc(disp_width * disp_hight * sizeof(lv_color_t));
    lv_color_t *buf_3_2 = lv_mem_alloc(disp_width * disp_hight * sizeof(lv_color_t));
    if (!buf_3_1) {
        printf("error in lvgl buf_3_1 malloc \n\n");
        return ;
    }
    if (!buf_3_2) {
        lv_mem_free(buf_3_1);
...
#define  buf_3_3 NULL
#else
    lv_color_t *buf_3_3 = lv_mem_alloc(disp_width * disp_hight * sizeof(lv_color_t));
    if (!buf_3_3) {
        lv_mem_free(buf_3_1);
        lv_mem_free(buf_3_2);
        buf_3_1 = NULL;
    ...
    disp_drv.hor_res = disp_width;//MY_DISP_HOR_RES;
    disp_drv.ver_res = disp_hight;//MY_DISP_VER_RES;

    /*Used to copy the buffer's content to the display*/
    disp_drv.flush_cb = disp_flush;

    disp_drv.rounder_cb = lvgl_disp_rounder_cb;
...
void lvgl_lcd_draw_task(void *p)
{
    while (1) {
        ...
        while (next_disp.fb != NULL) {
            ...
            if (disp_drv.full_refresh == 0) {
                lvgl_disp_flush(cur_disp.area.x1, cur_disp.area.x2, cur_disp.area.y1, cur_disp.area.y2, cur_disp.fb);
            } else {
                lvgl_disp_flush(0, disp_width, 0, disp_hight, cur_disp.fb);
            }
        }
    }
}
...
static void disp_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
    if (disp_flush_enabled) {
...
#if 1
            static unsigned int time_lapse_hdl;
            static unsigned char fps_cnt;
            ++fps_cnt;
            unsigned int tdiff = time_lapse(&time_lapse_hdl, 1000);
            if (tdiff) {
        ...
        lvgl_disp_flush(area->x1, area->x2, area->y1, area->y2, color_p);
