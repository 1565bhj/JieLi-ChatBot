...
static void direct_ui_update(void)
{
    ...
    // 更新标题
    if (music_info.title_changed && guider_ui.screen_music_label_title) {
        lv_label_set_text(guider_ui.screen_music_label_title, music_info.title);
...
#endif
// 更新标题，歌曲名
void music_update_title(char *title)
{
    if (!title) {
        return;
    }
    ...
    if (strcmp(music_info.title, title) != 0) {
        lv_snprintf(music_info.title, sizeof(music_info.title), "%s", title);
...
void async_screen_music_btn_play_pause_show(void* param)
{
    ...
    if (!is_music_page() || pause_status == is_pause) {
        return;
    }
    ...
    if (!is_pause) {
        ...
        if (guider_ui.screen_music_lyric) {
...
void volume_icon_click_handler(void)
{
    ...
    // 设置进度条的位置和尺寸
    lv_bar_set_value(guider_ui.screen_music_bar_volume, sys_volume_read(NULL), LV_ANIM_OFF);
}
/**
...
void async_music_page_init(void* param)
{
    ...
    case MODE_NET_ID:
        ...
        if (lv_demo_is_app_music_stop()) {
            lv_demo_music_play(1, NULL);
            async_screen_music_btn_play_pause_show(1);
        } else {
            async_screen_music_btn_play_pause_show(0);
        }
        return;
    ...
    lv_label_set_text(guider_ui.screen_music_lyric, g_music_current_lyrics);
    lv_label_set_text(guider_ui.screen_music_label_title, g_music_current_title);
...
void lv_demo_handle_music_pause(void)
{
    ...
    lv_demo_music_play_pause(1, NULL);
#endif
}
/**
 * 处理音乐播放功能
...
#ifdef APPLAYER_ENABLE
    void lv_demo_music_play_pause(char stop, char *type);
    lv_demo_music_play_pause(0, NULL);
#endif
}
/**
 * 处理上一曲播放功能
...
void lv_demo_handle_music_mode_switch(void)
{
    ...
    if (strcmp(get_current_play_mode_status(), "单曲循环") == 0) {
        lv_demo_music_play_loop(1);
    } else if (strcmp(get_current_play_mode_status(), "随机循环") == 0) {
        lv_demo_music_play_loop(0);
    }
#endif
}
