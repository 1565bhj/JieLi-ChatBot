...
typedef struct {
    ...
    int minute;
    int second;
} voice_alarm_delete_params_t;

/**
 * 异步语音删除闹钟回调函数
 * @param param: 语音删除闹钟参数结构体
 */
static void async_voice_delete_alarm(void *param)
{
    if (!param) {
        return;
    }

    voice_alarm_delete_params_t *delete_params = (voice_alarm_delete_params_t *)param;
    int hour = delete_params->hour;
    int minute = delete_params->minute;
    int second = delete_params->second;

    printf("-> 进入异步语音删除闹钟函数，删除时间: %02d:%02d:%02d\n", hour, minute, second);
 ...
 * @return: true表示删除操作已启动，false表示参数无效
 */
bool Voice_delete_alarm(int hour, int minute, int second)
{
    ...
    // 创建并初始化参数结构体
    voice_alarm_delete_params_t *params = (voice_alarm_delete_params_t *)malloc(sizeof(voice_alarm_delete_params_t));
    if (!params) {
        printf("-> 内存分配失败，无法删除闹钟\n");
        return false;
    }
    ...
    // 异步调用删除函数
    lv_async_call(async_voice_delete_alarm, (void *)params);
