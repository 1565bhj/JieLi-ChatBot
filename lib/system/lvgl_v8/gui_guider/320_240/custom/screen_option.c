...
static void  pressing_cb(lv_event_t * e)
{
    ...
    for (int32_t i=0;i<OPTION_ICON_COUNT;i++)
    {
        ...
        icon[i].x += t_offset_x;
        // 处理图标x坐标超出范围的情况（循环滚动）
        while(icon[i].x < (-OPTION_ICON_COUNT/2)*OPTION_ICON_DISTANCE){icon[i].x += (OPTION_ICON_COUNT)*OPTION_ICON_DISTANCE;}
        while(icon[i].x >  (OPTION_ICON_COUNT/2)*OPTION_ICON_DISTANCE){icon[i].x -= (OPTION_ICON_COUNT)*OPTION_ICON_DISTANCE;}
        // 设置图标对象的x坐标
        lv_obj_set_x(icon[i].obj,icon[i].x);
...
{
    // 处理x坐标超出范围的情况（循环滚动）
    while(v < (-OPTION_ICON_COUNT/2)*OPTION_ICON_DISTANCE){v += (OPTION_ICON_COUNT)*OPTION_ICON_DISTANCE;}
    while(v > (OPTION_ICON_COUNT/2)*OPTION_ICON_DISTANCE){v -= (OPTION_ICON_COUNT)*OPTION_ICON_DISTANCE;}
