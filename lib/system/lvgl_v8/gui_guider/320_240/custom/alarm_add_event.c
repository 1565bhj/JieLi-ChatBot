...
static void async_ui_dynamic_add_new_alarm(void* param) {
    ...
    // 根据参数创建新闹钟
    Dynamic_add_new_alarm(alarm_params->ui, alarm_params->hour, alarm_params->minute, 0, alarm_params->repeat_status);
    // 释放参数内存
    free(param);
}
// 同步调用异步添加闹钟的函数
void Sync_ui_dynamic_add_new_alarm(lv_ui *ui, uint8_t hour, uint8_t minute, int alarm_index, bool repeat_status) {
    // 创建并初始化参数结构体
...
// 同步调用异步添加闹钟的函数
void Sync_app_dynamic_add_new_alarm(lv_ui *ui, uint8_t hour, uint8_t minute, int alarm_index, bool repeat_status) {
    // 创建并初始化参数结构体
    alarm_add_params_t *params = (alarm_add_params_t *)malloc(sizeof(alarm_add_params_t));
    if (params) {
        params->ui = ui;
...
static void alarm_add_yes_event_handler(lv_event_t *e) {
    ...
    if(code == LV_EVENT_CLICKED) {
        ...
        // 同步调用添加闹钟函数
        Sync_ui_dynamic_add_new_alarm(ui, hour, min, 0, g_alarms[g_current_alarm_index].repeat_status);
        // 同步调用添加闹钟函数
        Sync_app_dynamic_add_new_alarm(ui, hour, min, g_current_alarm_index, g_alarms[g_current_alarm_index].repeat_status);
                // 切换回闹钟主页面
