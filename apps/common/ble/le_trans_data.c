...
static u8 rsp_info_cmd_le[512] = {0};
int Total_size = 0;
static int _ble_send_device_info(void)
{
    int ret;
    int Length = 0;
    u16 crc;
    ...
    //获取BLE MAC地址
    bt_ble_get_mac(mac);
    memset(rsp_info_cmd_le, 0, sizeof(rsp_info_cmd_le));

    // 初始化协议头
    rsp_info_cmd_le[0] = 0x4A;  // "J"
    rsp_info_cmd_le[1] = 0x4C;  // "L"
    rsp_info_cmd_le[4] = 0x10;  // 一级命令
    rsp_info_cmd_le[5] = 0x02;  // 二级命令

    Total_size = 6;  // 协议头固定6字节

    // 从设备读取信息（这里需要实现lte_dev_info_opt函数）
    // lte_dev_info_opt(1, iccid_0, iccid_1, lte_mac, imei);

    // 生成JSON格式数据
    ret = sprintf(&rsp_info_cmd_le[Total_size], "{\"status\":0,\"mac\":\"%02X%02X%02X%02X%02X%02X\"",
                  mac[5], mac[4], mac[3], mac[2], mac[1], mac[0]);
    Total_size += ret;
    Length += ret;

    // 添加UUID信息
    char *uuid = "eZ3zJIju"; // 替换为实际的UUID获取函数
    if (uuid) {
        ret = sprintf(&rsp_info_cmd_le[Total_size], ",\"uuid\":\"%s\"", uuid);
        Total_size += ret;
        Length += ret;
    }

    // 添加批次号
    char *batch_num = "B1N1-YHY4G"; // 替换为实际的批次号获取函数
    if (batch_num) {
        ret = sprintf(&rsp_info_cmd_le[Total_size], ",\"batch_num\":\"%s\"", batch_num);
        Total_size += ret;
        Length += ret;
    }

    // 添加ICCID信息
    ret = sprintf(&rsp_info_cmd_le[Total_size], ",\"iccid_0\":\"%s\"", "898608692325d0265977");
    Total_size += ret;
    Length += ret;

    ret = sprintf(&rsp_info_cmd_le[Total_size], ",\"iccid_1\":\"%s\"", "89860624640008050197");
    Total_size += ret;
    Length += ret;

    // 添加LTE MAC和IMEI
    ret = sprintf(&rsp_info_cmd_le[Total_size], ",\"lte_mac\":\"%s\"", "f0cbf2792277");
    Total_size += ret;
    Length += ret;

    ret = sprintf(&rsp_info_cmd_le[Total_size], ",\"imei\":\"%s\"", "869778074932865");
    Total_size += ret;
    Length += ret;

    //闭合JSON
    ret = sprintf(&rsp_info_cmd_le[Total_size], "}");
    if (ret > 0) {
        Total_size += ret;
        Length += ret;  // 闭合符(1字节)
        Length += 2;    // 协议头长度字段的2字节

        // 设置长度字段
        rsp_info_cmd_le[2] = (Length >> 8) & 0xFF;
        rsp_info_cmd_le[3] = Length & 0xFF;

        // 计算CRC
        crc = CRC16(&rsp_info_cmd_le[2], Length + 2);
        rsp_info_cmd_le[Total_size++] = (crc >> 8) & 0xFF;
        rsp_info_cmd_le[Total_size++] = crc & 0xFF;
        rsp_info_cmd_le[Total_size++] = 0xFF;  // 结束符
        printf("rsp_mac_cmd: %d-%d-%s", rsp_info_cmd_le[2] * 256 + rsp_info_cmd_le[3], Total_size, &rsp_info_cmd_le[6]);
    }
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    log_info("read_callback, handle= 0x%04x, buffer=%08x, buffer_size=%d\n", handle, (u32)buffer, buffer_size);
    switch (handle) {
    ...
    case ATT_CHARACTERISTIC_ae10_01_VALUE_HANDLE:
        ...
        //第二次回调，buffer才不为空
        if (buffer) {
            memcpy(buffer, &rsp_info_cmd_le[offset], buffer_size);
            //返回实际拷贝的数据长度
            att_value_len = buffer_size;
        }
        break;
