...
#endif
//#define GUI_GIF_INPUT_FILE_ENABLE //使用文件系统读取
...
static int GUI_GIF__GetData(void *p, const u8 **ppData, unsigned NumBytesReq, u32 Offset)
{
    ...
    RemBytes = pParam->DataSize - Offset;
    NumBytes = 0;
    if (pParam->fd) {
    ...
    if (RemBytes > 0) {
        ...
        if (pParam->fd) {
            if (!pParam->pFileData) {
                pParam->DataSize = 0;
                pParam->DataSizeValid = 0;
                pParam->pFileData = malloc(NumBytes);
                if (!pParam->pFileData) {
                    printf("malloc error\n");
                    return 0;
                }
                pParam->DataSize = NumBytes;
            } else if (NumBytes > pParam->DataSize) {
                free(pParam->pFileData);
                pParam->DataSize = 0;
                pParam->DataSizeValid = 0;
                pParam->pFileData = malloc(NumBytes);
                if (!pParam->pFileData) {
            ...
            if (Offset == 0) {
                pParam->FileOffset = 0;
                pParam->DataSizeValid = 0;
                fseek(pParam->fd, Offset, SEEK_SET);
            }
            if (pParam->DataSizeValid == 0) {
                readsize = NumBytes;
                if (fread(pParam->pFileData, readsize, 1, pParam->fd) != readsize) {
                    printf("fread error\n");
                    return 0;
                }
                pParam->DataSizeValid = readsize;
                pParam->FileOffset = Offset;
            }
            if (Offset >= pParam->FileOffset && (Offset + NumBytes) <= (pParam->FileOffset + pParam->DataSizeValid)) {
                *ppData = pParam->pFileData + (Offset - pParam->FileOffset);
            } else {
                pParam->FileOffset = Offset;
                fseek(pParam->fd, Offset, SEEK_SET);
                readsize = NumBytes;
                ...
                pParam->DataSizeValid = readsize;
                *ppData = pParam->pFileData;
            }
#else
            fseek(pParam->fd, Offset, SEEK_SET);
            if (fread(pParam->pFileData, NumBytes, 1, pParam->fd) != NumBytes) {
                printf("fread error\n");
                return 0;
            }
            *ppData = pParam->pFileData;
...
#ifndef GUI_GIF_INPUT_FILE_ENABLE
/*********************************************************************
*
*       GUI_GIF_GetInfo
*/
...
#endif // GUI_GIF_INPUT_FILE_ENABLE

/*********************************************************************
*
*       GUI_GIF_GetInfoFileOpen
*/
int GUI_GIF_GetInfoFileClose(GUI_GIF_IMAGE_INFO *pInfo);

int GUI_GIF_GetInfoFileOpen(const char *path, GUI_GIF_IMAGE_INFO *pInfo)
{
    int r = 1;
    static GUI_GIF_PARAM param ALIGNED(4) = {0};

    GUI_GIF_GetInfoFileClose(pInfo);

    param.fd = fopen(path, "rb");
    if (!param.fd) {
        printf("-->gif fopen err : %s\n", path);
        return -1;
    }
    param.FileSize = flen(param.fd);
    param.DataSize = 8192;
    param.DataSizeValid = 0;
    param.pFileData = malloc(param.DataSize);
    if (!param.pFileData) {
        param.DataSize = 0;
    }
    printf("gif file %d : %s\n", param.FileSize, path);
...
/*********************************************************************
*
*       GUI_GIF_GetInfoFileClose
*/
int GUI_GIF_GetInfoFileClose(GUI_GIF_IMAGE_INFO *pInfo)
{
    int r = 1;
    GUI_GIF_PARAM *param = (GUI_GIF_PARAM *)pContext.pParam;
    ...
    if (param) {
        ...
        if (param->pFileData) {
            free(param->pFileData);
            param->pFileData = NULL;
            param->DataSize = 0;
            param->DataSizeValid = 0;
        }
    }
...
#ifdef GUI_GIF_INPUT_FILE_ENABLE
int Gif_to_PictureFile(GUI_GIF_IMAGE_INFO *Info, char *inbuf, int type, int Index)
{
    GUI_GIF_PARAM *param = (GUI_GIF_PARAM *)pContext.pParam;
...
/*
    printf("--> line = %d\n",__LINE__);
    if (GUI_GIF_GetInfoFileOpen(gif_path, &info)) {
        printf("GIF file type err\n");
        goto _exit;
    }
    if(Gif_to_PictureFile(&info, rgb565_buf, 2, i)){
