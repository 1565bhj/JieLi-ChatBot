...
#if USE_LCD_TE
static OS_SEM lcd_te_sem;
static void (*te_interrupt_cb)(void*);
static void lcd_te_interrupt_callback(void)
{
    os_sem_post(&lcd_te_sem);
}
int lcd_te_interrupt_init(int failing_edge)
{
    os_sem_create(&lcd_te_sem, 0);
    if (lcd_pdata->te_pin != (u8) -1) {
        int ret = port_wakeup_reg(EVENT_IO_1, lcd_pdata->te_pin, failing_edge, lcd_te_interrupt_callback);
        if (ret) {
            printf("te_io success.\n");
        } else {
            printf("te_io fail.\n");
        }
    }
...
int lcd_te_interrupt_waite(void)
{
    int ret = 0;
    os_sem_set(&lcd_te_sem, 0);
    ret = os_sem_pend(&lcd_te_sem, 50);
    if (ret) {
        printf("te_io waite err\n\n");
    }
    return ret;
}
#endif // USE_LCD_TE
...
void lcd_rst_pinstate(u8 state)
{
    if (lcd_pdata->rst_pin != (u32) - 1) {
        gpio_set_pull_up(lcd_pdata->rst_pin, 1);
        gpio_set_pull_down(lcd_pdata->rst_pin, 0);
...
{
    if (lcd_pdata->cs_pin != (u32) - 1 && ((lcd_cs_pin & BIT(0)) || lcd_cs_pin == 0)) {
        gpio_set_pull_up(lcd_pdata->cs_pin, 1);
        gpio_set_pull_down(lcd_pdata->cs_pin, 0);
        gpio_direction_output(lcd_pdata->cs_pin, state);
    }
#ifdef CONFIG_UI_TOW_EYE
    if (lcd_pdata->cs1_pin != (u32) - 1 && (lcd_cs_pin & BIT(1))) {
        gpio_set_pull_up(lcd_pdata->cs1_pin, 1);
        gpio_set_pull_down(lcd_pdata->cs1_pin, 0);
        gpio_direction_output(lcd_pdata->cs1_pin, state);
    }
#endif
}
...
void lcd_rs_pinstate(u8 state)
{
    if (lcd_pdata->rs_pin != (u32) - 1) {
        gpio_set_pull_up(lcd_pdata->rs_pin, 1);
        gpio_set_pull_down(lcd_pdata->rs_pin, 0);
...
#ifdef TCFG_LCD_BL_PWM_PORT
        static void *bl_pwm = NULL;
        static struct pwm_platform_data bl_pwm_data = {0};
        static float default_duty = 0;
        if (!bl_pwm) {
            bl_pwm = dev_open("pwm1", &bl_pwm_data);
            if (!bl_pwm) {
                printf("warning open lcd bl pwm dev err !!!!\n");
            }
            default_duty = bl_pwm_data.duty;
            if (user_lcd_light_read()) {
                bl_pwm_data.duty = user_lcd_light_read();
                bl_pwm_data.duty = MIN(100, bl_pwm_data.duty);
            }
        }
        if (bl_pwm) {
            if (!bl_pwm_data.duty && state) {
                bl_pwm_data.duty = user_lcd_light_read();
                bl_pwm_data.duty = MIN(100, bl_pwm_data.duty);
                bl_pwm_data.duty = bl_pwm_data.duty > 0 ? bl_pwm_data.duty : default_duty;
            }
            bl_pwm_data.duty = state == 1 ? (bl_pwm_data.duty ? bl_pwm_data.duty : default_duty) : state;
            if (user_lcd_light_read() != (int)bl_pwm_data.duty && bl_pwm_data.duty != 0) {
                user_lcd_light_write((int)bl_pwm_data.duty);
            }
            dev_write(bl_pwm, (void *)&bl_pwm_data, 0);
        }
#else
...
//        dev_ioctl(lcd_hdl, IOCTL_SPI_NON_BLOCK, 1);//设置为非阻塞发送数据包
        dev_ioctl(lcd_hdl, IOCTL_SPI_NON_BLOCK, 0);//设置为非阻塞发送数据包
...
//            dev_ioctl(lcd_hdl, IOCTL_EMI_WRITE_NON_BLOCK, 1);
            dev_ioctl(lcd_hdl, IOCTL_EMI_WRITE_NON_BLOCK, 0);
        }
    }
...
void lcd_set_send_noblobk(int noblock)
{
    if (lcd_hdl) {
        dev_ioctl(lcd_hdl, IOCTL_SPI_NON_BLOCK, noblock);//1设置为非阻塞发送数据包
    }
}
...
void lcd_show_set_dirction(int angle)
{
    if (!lcd_dev) {
        return ;
    }
    if (lcd_dev->set_dirction) {
        lcd_dev->set_dirction(angle);
    }
}
void lcd_show_set_dirction_default(int angle)
{
    if (!lcd_dev) {
        return ;
    }
    if (lcd_dev->set_dirction) {
        lcd_dev->set_dirction_default(angle);
    }
}
void lcd_show_mirror1_set(char enable)//另外眼睛镜像
{
    if (!lcd_dev) {
        return ;
    }
    os_mutex_pend(&mutex, 2000);
    u8 temp = lcd_cs_pin;
    lcd_cs_pin = BIT(1);
    if (lcd_dev->LCD_Mirror1) {
        lcd_dev->LCD_Mirror1(enable);
    }
    lcd_cs_pin = temp;
    os_mutex_post(&mutex);
}
...
/*
*该接口显示数据直接推送数据到LCD设备接口，可以采用翻转角度:一般设置为0(数据)，设置翻转为0加上TE信号则可解决撕裂感
*但需要注意：屏幕默认的全0则可能是横屏或者竖屏，竖屏当横屏（横屏当竖屏）使用，则需要在发送前把数据进行翻转，否则显示异常
*/
void lcd_show_frame_to_dev_flip(u8 *buf, u32 len, int angle)
{
    os_mutex_pend(&mutex, 2000);
    lcd_show_set_dirction(angle);
    lcd_draw_to_dev(buf, len);
    os_mutex_post(&mutex);
}
...
{
    os_mutex_pend(&mutex, 500);
    user_draw(x, y, w, h, img);
    os_mutex_post(&mutex);
}
void lcd_lvgl_able(char enable)
{
    lvgl_enable = enable;
}
void lcd_lvgl_full(u16 xs, u16 xe, u16 ys, u16 ye, u8 *img)
{
    ...
    os_mutex_pend(&mutex, 500);
    lcd_show_set_dirction_default(0);
...
#endif
#ifdef CONFIG_UI_PLAY_EMOJI
        play_face_emoji_lock(1);
#endif

        memset(buf, 0, len);
        unsigned short color[] = {RED, MAGENTA, YELLOW, GREEN, CYAN, BLUE, PURPLE, BLACK, WHITE};
        int cnt = sizeof(color) / sizeof(color[0]);
        int width = LCD_W;
        int height = LCD_H;
#ifdef USE_LCD_TE_ROTATE_ENABLE
        width = LCD_H;
        height = LCD_W;
#endif
        for (int n = 0; n < 2; n++) {
            ...
            for (int i = 0; i < height; i++) {
                for (int j = 0; j < width; j++) {
                    *data++ = (color[i / (height / cnt)] >> 8) & 0xff;
                    *data++ = color[i / (height / cnt)] & 0xff;
                }
            }
            lcd_show_frame_to_dev(buf, len);
            os_time_dly(100);
            memset(buf, 0, len);
            data = buf;
            for (int i = 0; i < height; i++) {
                for (int j = 0; j < width; j++) {
                    *data++ = (color[j / (width / cnt)] >> 8) & 0xff;
                    *data++ = color[j / (width / cnt)] & 0xff;
                }
            }
            lcd_show_frame_to_dev(buf, len);
            os_time_dly(100);
