...
void play_gif_to_lcd(char *gif_path, char play_speed)
{
    ...
    if (!gif_fd) {
        printf("[error]gif_file_open_fail : %s \n", gif_path);
        return;
    }
    ...
    if (info.xSize <= lcd_w && info.ySize <= lcd_h) {
        ...
        printf("[error] %d %d, %d %d", info.xSize, lcd_w, info.ySize, lcd_h);
        return ;
    }
    ...
    while (1) {
        for (u16 i = 0; i < info.NumImages; i++) {
            if (Gif_to_Picture(buf, gif_len, &info, rgb565_buf, 2, i)) {
...
//            printf("--> i = %d\n",i);
            ...
            //lcd_lvgl_full(0, info.xSize, 0, info.ySize, rgb565_buf);
            void ST77912_Shown_Image(u8 * buff, u16 x_addr, u16 y_addr, u16 width, u16 height);
            ST77912_Shown_Image(rgb565_buf, 0, 0, lcd_w, lcd_h);
//            FILE *fd = fopen(CONFIG_ROOT_PATH"1.rgb", "wb+");
//            if(fd){
//                fwrite(rgb565_buf, info.xSize * info.ySize * 2, 1, fd);
//                fclose(fd);
//            }else{
//                printf("--> fopen err\n");
//            }
