...
//static wireless_device_t wireless_dev[USB_MAX_HW_NUM];
struct wireless_device_t wireless_dev[USB_MAX_HW_NUM];
...
const struct usb_interface_info at_port_inf[USB_MAX_HW_NUM] = {
    {
...
#define MAX_HARDWARE_ERROR_CNT (4)
#define MAX_TRANSFER_ERROR_CNT (500)
static volatile int usb_net_hd_reset_flag = 0;
static volatile int rx_err_cnt = 0;
static volatile int at_rx_err_cnt = 0;
static volatile int bulk_send_err_cnt = 0;

//1：上层操作硬件 2：底层硬件出错
int usb_net_get_module_reset_status(void)
{
    return usb_net_hd_reset_flag;
}

//set的操作放到上层
void usb_net_set_module_reset_status(int val)
{
    usb_net_hd_reset_flag = val;
}

int usb_net_get_err_cnt(void)
{
    return (rx_err_cnt);
}

int usb_net_get_at_err_cnt(void)
{
    return (at_rx_err_cnt);
}

int usb_net_set_err_cnt(int val)
{
    rx_err_cnt = val;
    return 0;
}

int usb_net_set_at_err_cnt(int val)
{
    at_rx_err_cnt = val;
    return 0;
}
...
{
    if (usb_net_get_module_reset_status() == 1) {
        printf("4G-LTE module has been reset!!!,lien:%d", __LINE__);
        return -1;
    } else if (usb_net_get_module_reset_status() == 2) {
        printf("4G-LTE need to reset!!!,lien:%d", __LINE__);
//        usb_host_force_reset(usb_id);
    ...
    //没有遍历到可用的接口
    if (!wireless_dev[usb_id].epout_buf) {
        printf("usbnet_host_bulk_only_send fail!!!");
        return -1;
    }

    //printf("usbnet_host_bulk_only_send:%d, 0x%02x, 0x%02x， 0x%02x=================================", usb_id, wireless_dev[usb_id].host_epout, wireless_dev[usb_id].txmaxp, wireless_dev[usb_id].epout);
    int ret = usb_h_bulk_write(usb_id, wireless_dev[usb_id].host_epout, wireless_dev[usb_id].txmaxp, wireless_dev[usb_id].epout, buf, len);
    if (ret < 0) {
        bulk_send_err_cnt++;
        printf("usbnet_host_bulk_only_send out:%d, bulk_send_err_cnt:%d!!!", ret, bulk_send_err_cnt);
        if (bulk_send_err_cnt >= MAX_TRANSFER_ERROR_CNT) {
            printf("need to usb_host_unmount_3");
            bulk_send_err_cnt = 0;
            usb_net_set_module_reset_status(2);
        }
    } else {
        bulk_send_err_cnt = 0;
    }
    //printf("dat-out:%d", ret);
    return ret;
}
...
int usbnet_host_at_data_send(usb_dev usb_id, u8 *buf, u32 len)
{
    ...
    //没有遍历到可用的接口
    if (!wireless_dev[usb_id].epout_buf_at) {
        printf("usbnet_host_at_data_send fail, usb_id=%d!!!", usb_id);
        return -1;
    }

    char tmp_buf[512];
    if (len >= sizeof(tmp_buf)) {
        printf("tmp_buf is not enought!!!");
        len = sizeof(tmp_buf) - 1;
    }
    memcpy(tmp_buf, buf, len);
    tmp_buf[len] = '\0';

    //printf("usbnet_host_at_data_send:%d, 0x%02x, 0x%02x, 0x%02x, len=%d=================================", usb_id, wireless_dev[usb_id].host_epout_at, wireless_dev[usb_id].txmaxp_at, wireless_dev[usb_id].epout_at, len);
    printf("send at cmd:%s", tmp_buf);
    int ret = usb_h_bulk_write(usb_id, wireless_dev[usb_id].host_epout_at, wireless_dev[usb_id].txmaxp_at, wireless_dev[usb_id].epout_at, buf, len);
    if (ret < 0) {
        bulk_send_err_cnt++;
        printf("usbnet_host_at_data_send out:%d, bulk_send_err_cnt:%d!!!", ret, bulk_send_err_cnt);
        if (bulk_send_err_cnt >= MAX_TRANSFER_ERROR_CNT) {
            printf("need to usb_host_unmount_4");
            bulk_send_err_cnt = 0;
            usb_net_set_module_reset_status(2);
        }
    } else {
...
//fix bug
int usb1_rx_err_handle(int rxlen)
{
    printf("usb1_rx_err_handle:%d, rx_err_cnt:%d", rxlen, rx_err_cnt);
    rx_err_cnt++;
    if (rx_err_cnt >= MAX_HARDWARE_ERROR_CNT) {
        printf("need to usb_host_unmount_1");
        usb_net_set_module_reset_status(2);
    }
...
static void usbnet_rx_isr(struct usb_host_device *host_dev, u32 ep)
{
    ...
    //printf("usbnet_rx_isr:0x%02x======", USB_DIR_IN|target_ep);
    ...
    //fix bug
    if (rx_len == -DEV_ERR_TIMEOUT) {
        rx_err_cnt++;
        printf("hardware error, rx_len:%d, rx_err_cnt:%d+++++++++++++!!!", rx_len, rx_err_cnt);
        if (rx_err_cnt >= MAX_HARDWARE_ERROR_CNT) {
            printf("need to usb_host_unmount_0");
            usb_net_set_module_reset_status(2);
        }
    } else {
        rx_err_cnt = 0;
...
void usbnet_at_port_rx_handler_unregister()
{
    usbnet_at_port_rx_handler = NULL;
}

//static void usbnet_at_port_rx_isr(struct usb_host_device *host_dev, u32 ep);
void usbnet_at_port_rx_isr(struct usb_host_device *host_dev, u32 ep)
{
    int rx_len = 0;
    usb_dev usb_id = host_device2id(host_dev);
...
//    printf("usb_h_ep_read_async03\r\n");
//    printf("r-03\r\n");
    //printf("usbnet_at_port_rx_isr:0x%02x======", USB_DIR_IN|target_ep);
    rx_len = usb_h_ep_read_async(usb_id, ep, target_ep, usb_recv_buf, usb_recv_len, USB_ENDPOINT_XFER_BULK, 0);
    //fix bug
    if (rx_len == -DEV_ERR_TIMEOUT) {
        at_rx_err_cnt++;
        printf("hardware error(at), rx_len:%d, at_rx_err_cnt:%d+++++++++++++!!!", rx_len, at_rx_err_cnt);
        if (at_rx_err_cnt >= MAX_HARDWARE_ERROR_CNT) {
            printf("need to usb_host_unmount_a");
            usb_net_set_module_reset_status(2);
        }
        return;
    } else if (rx_len > 0) {
    ...
    if (rx_len > 0) {
        if (usbnet_at_port_rx_handler) {
            ...
            printf("usbnet_at_port_rx_handler is NULL!!!");
            return;
        }
    }
//    printf("usb_h_ep_read_async04\r\n");
//    printf("r-04\r\n");
    usb_h_ep_read_async(usb_id, ep, target_ep, NULL, 0, USB_ENDPOINT_XFER_BULK, 1);
}
...
{
    printf("*****************************usbnet_wireless_interface_parser, interface_num:%d*****************************", interface_num);
    int len = 0;
    u8 rx_interval = 0;
    const usb_dev usb_id = host_device2id(host_dev);
...
__set_config:
                ...
                printf("wireless_dev[%d].epin:0x%02x", usb_id, wireless_dev[usb_id].epin | USB_DIR_IN);
                printf("wireless_dev[%d].epout:0x%02x", usb_id, wireless_dev[usb_id].epout);
                printf("wireless_dev[%d].txmaxp:0x%02x", usb_id, wireless_dev[usb_id].txmaxp);
                printf("wireless_dev[%d].rxmaxp:0x%02x", usb_id, wireless_dev[usb_id].rxmaxp);
