...
static int set_flash_wifi_mac(char *mac)
{
    ...
    ret = user_rf_fcc_info_write(CFG_BT_MAC_ADDR, mac, 6, mac_trim);
    if (ret) {
...
int wifi_set_flash_mac(char *mac)
{
    ...
    mac_trim = true;
#endif
    return set_flash_wifi_mac(mac);
}
static int get_flash_mac_addr(char mac[6])
{
    if (user_rf_fcc_info_read(CFG_BT_MAC_ADDR, mac, 6)) {
        return mac;
    }
...
int init_net_device_mac_addr(char *macaddr, char ap_mode)
{
    ...
    if (-1 == get_flash_mac_addr(macaddr)) {
        if (first) {
            ...
            macaddr[0] &= ~((1 << 0) | (1 << 1));//MAC第一个字节的0和1位需要为0才是唯一性
            while (macaddr[0] == 0) {//去除第一个和最后一个为0
                macaddr[0] = (char *)(JL_RAND->R64L ^ CRC16(flash_uid, sizeof(flash_uid)));
                macaddr[0] &= ~((1 << 0) | (1 << 1));
            }
            while (macaddr[5] == 0) {//去除第一个和最后一个为0
                macaddr[5] = (char *)(JL_RAND->R64L ^ CRC16(flash_uid, sizeof(flash_uid)));
            }
...
int bt_ble_get_mac(char *mac)
{
    return wifi_get_flash_mac(mac);
}
#endif
#endif
