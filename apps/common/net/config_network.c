...
void config_network_start(void)
{
	...
	net_dhcp_status_set(NET_DHCP_INIT);
...
{
    if (!config_network_flag) {
        return;
    }
    printf("=====config_network_stop=====");
...
int bt_net_config_set_ssid_pwd_get(char **ssid, char **pwd, char clean)
{
    int ret = 0;
    if (clean) {
        memset(bt_net_result.pwd, 0, sizeof(bt_net_result.pwd));
        memset(bt_net_result.ssid, 0, sizeof(bt_net_result.ssid));
        return ret;
    }
    if (bt_net_result.ssid[0]) {
        ret = true;
        if (ssid) {
            *ssid = bt_net_result.ssid;
        }
        if (pwd) {
            *pwd = bt_net_result.pwd;
        }
    }
    return ret;
}
...
int bt_net_config_set_ssid_pwd(const char *data)
{
    ...
    const char *str_ssid, *str_pwd, *str_account, *net_status;
...
//        sys_net_channel_ssid_write(bt_net_result.ssid, bt_net_result.pwd);
//#endif
        //用户账户信息(轻语号)
        str_account = json_object_get_string(json_object_object_get(json, "user_account"));
        if (str_account) {
            printf("bt net config user_account : %s\n", str_account);
        }

        //配网结果状态包
        net_status = json_object_get_string(json_object_object_get(json, "net_status"));
        if (net_status) {
            if (strstr(net_status, "timeout")) { //{"net_status": "timeout"}
				json_object_put(json);
                printf("WIFI network configuration timeout:%s\n", net_status);
                net.arg = "net";
