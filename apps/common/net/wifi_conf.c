...
const char WL_RX_OVERFLOW_DEBUG = 1; //统计WIFI底层接收FIFO塞满导致丢包打印,一般认为对端发送太猛/空中干扰太强/CPU太繁忙来接收线程来不及取数因素导致, 使能后如果出现丢包打印每秒丢多少个数据包
...
const u16 MAX_PACKETS_IN_PS_QUEUE   = 32; //配置WiFi驱动最大发送power-save队列    //128   /*16 */
#endif
...
u8 IPV4_ADDR_CONFLICT_DETECT = 0;  //4G关闭静态ip冲突检测
#elif (defined CONFIG_LTE_PHY_ENABLE)
const u8 IPV4_ADDR_CONFLICT_DETECT = 0;  //4G关闭静态ip冲突检测
#elif (defined CONFIG_WIFI_ENABLE)
const u8 IPV4_ADDR_CONFLICT_DETECT = 1;  //WiFi开启静态ip冲突检测
#else
...
static u8 wifi_work_mode = WIFI_MODE_NONE;
void wl_wifi_cur_mode_set(u8 wifi_mode)
{
    wifi_work_mode = WIFI_MODE_AP;
}

u8 wl_wifi_cur_mode_get(void)
{
    return wifi_work_mode;
}

void wl_wifi_cur_mode_clear(void)
{
...
#if 0
WirelessMode:
PHY_11BG_MIXED = 0,//暂时使用bg模式20M，n模会触发20/40M共存路由器下发切换帧出现信道切换无法切回问题
PHY_11B = 1,
PHY_11A = 2,,
PHY_11ABG_MIXED = 3,,
PHY_11G = 4,
