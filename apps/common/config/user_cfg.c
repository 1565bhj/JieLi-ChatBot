...
const struct btif_item btif_table[] = {
    ...
    {CFG_BT_RF_POWER_ID,        1},
    {CFG_BLE_RF_POWER_ID,       1},

//#ifdef RF_FCC_DATA_TRIM_ENABLE
//    {VM_BT_MAC_ADDR,           6 },
//    {VM_XOSC_INDEX,            2 },
//    {VM_WIFI_PA_DATA,           sizeof(wifi_calibration_param.pa_trim_data)},
//    {VM_WIFI_PA_MCS_DGAIN,      sizeof(wifi_calibration_param.mcs_dgain)},
//#endif
...
int __attribute__((weak)) user_rf_fcc_info_write(int index, unsigned char *data, int size, char trim)//用户字节写校准值，返回值：实际的读写字节数
{
    switch (index) { //有读写的返回实际读写的字节长度
    case CFG_BT_MAC_ADDR:
        if (trim) {
            int ret = syscfg_write(CFG_BLE_MAC_ADDR, data, size);
            printf("write user mac : %d, size: %d\n", ret, size);
            if (ret != size) {
                return 0;
            } else {
                return ret;
            ...
            printf("write user mac not trim: size: %d\n", size);
        }

        break;
    case VM_XOSC_INDEX:
        break;
    case VM_WIFI_PA_DATA:
        break;
    case VM_WIFI_PA_MCS_DGAIN:
        break;
    }
    return 0;
}
int __attribute__((weak)) user_rf_fcc_info_read(int index, unsigned char *data, int size)//用户字节读校准值，返回值：实际的读写字节数
{
    switch (index) { //有读写的返回实际读写的字节长度
    case CFG_BT_MAC_ADDR:
        int ret = syscfg_read(CFG_BLE_MAC_ADDR, data, size);
        printf("read user mac : %d, size: %d\n", ret, size);
        if (ret != size) {
            return 0;
        } else {
            put_buf(data, ret);
            if ((data[0] == 0xFF) && (data[1] == 0xFF) && (data[2] == 0xFF)) {
                printf("read invalid mac FF:FF:FF:FF:FF:FF\n");
                return 0;
            }

            if ((data[0] == 0x00) && (data[1] == 0x00) && (data[2] == 0x30) && (data[0] == 0x00) && (data[1] == 0x00) && (data[2] == 0x00)) {
                printf("read invalid mac 00:00:30:00:00:00\n");
                return 0;
            }
            return ret;
        }
...
const u8 *bt_get_mac_addr(void)
{
    ...
    const u8 pdt_mac[6] = {0x23, 0x84, 0x32, 0xCC, 0x48, 0xAD};

    if (is_production_test_enter(0)) {
        return pdt_mac;
    }
    if (user_rf_fcc_info_read(CFG_BT_MAC_ADDR, mac_addr, 6)) {
        return mac_addr;
    }
    ...
    if (bytecmp(mac_addr, 0, 6) && bytecmp(mac_addr, 0xFF, 6)) {
        return mac_addr;
    } else
#endif
    {
        ...
        if (user_rf_fcc_info_write(CFG_BT_MAC_ADDR, mac_addr, 6, 0)) {
            norflash_protect_resume();
            return mac_addr;
        }
#ifndef RF_FCC_DATA_TRIM_ENABLE
        syscfg_write(CFG_BT_MAC_ADDR, mac_addr, 6);
#endif
        norflash_protect_resume();
        printf("flash rand mac save\n");
        put_buf(mac_addr, 6);
        return mac_addr;
    }
}
...
//    sprintf(edr_name, "JL-AC791N-%02X%02X", mac_addr[4], mac_addr[5]);
#ifdef BT_DEV_NAME_LONE_ENABLE
    sprintf(edr_name, "%s%02X%02X%02X%02X%02X%02X", BT_DEV_NAME, mac_addr[5], mac_addr[4], mac_addr[3], mac_addr[2], mac_addr[1], mac_addr[0]);
#else
    sprintf(edr_name, "%s%02X%02X", BT_DEV_NAME, mac_addr[4], mac_addr[5]);
#endif
    if (is_production_test_enter(0)) {
#ifdef PRODUCTION_ALL_TEST_BT_NAME
        sprintf(edr_name, "%s%s", BT_DEV_NAME, PRODUCTION_ALL_TEST_BT_NAME);
#endif
    }
    printf("-> bt_name : %s \n", edr_name);
#endif
...
void cfg_file_parse(void)
{
    ...
    //-----------------------------CFG_BT_MAC_ADDR--------------------------------------//
    if (!user_rf_fcc_info_read(CFG_BT_MAC_ADDR, tmp, 6)) {
    ...
    fin_ble_power = ble_power;
    fin_bt_power = bt_power;
...
#if defined CONFIG_BT_ENABLE || defined CONFIG_WIFI_ENABLE
int wf_btif_xosc_write(u8 *xosc)
{
    if (xosc) {
        norflash_protect_suspend();//解除flash写保护
...
//#endif
    if (user_rf_fcc_info_read(VM_XOSC_INDEX, xosc, 2)) {
        return ;
    } else {
        norflash_protect_suspend();//解除flash写保护
        if (user_rf_fcc_info_write(VM_XOSC_INDEX, xosc, 2, 0)) {
            norflash_protect_resume();
            return ;
        }
        norflash_protect_resume();
    ...
    if (syscfg_read(VM_XOSC_INDEX, xosc, 2) != 2) {
        //wf_btif_xosc_write(xosc);
    }
    printf("->wifi_get_xosc xosc ：%d , %d \n", xosc[0], xosc[1]);
}

int wf_btif_mcs_write(u8 *mcs_dgain)
{
    if (mcs_dgain) {
        norflash_protect_suspend();//解除flash写保护
...
//#endif
    if (user_rf_fcc_info_read(VM_WIFI_PA_MCS_DGAIN, mcs_dgain, sizeof(wifi_calibration_param.mcs_dgain))) {
        return ;
    } else {
        norflash_protect_suspend();//解除flash写保护
        if (user_rf_fcc_info_write(VM_WIFI_PA_MCS_DGAIN, mcs_dgain, sizeof(wifi_calibration_param.mcs_dgain), 0)) {
            norflash_protect_resume();
            return ;
        }
        norflash_protect_resume();
    ...
    if (syscfg_read(VM_WIFI_PA_MCS_DGAIN, mcs_dgain, sizeof(wifi_calibration_param.mcs_dgain)) != sizeof(wifi_calibration_param.mcs_dgain)) {
        //wf_btif_mcs_write(mcs_dgain);
    }
    printf("->wifi_get_mcs_dgain \n");
    put_buf(mcs_dgain, sizeof(wifi_calibration_param.mcs_dgain));
}

int wf_btif_pa_trim_write(u8 *pa_data)
{
    if (pa_data) {
        norflash_protect_suspend();//解除flash写保护
...
//#endif
    if (user_rf_fcc_info_read(VM_WIFI_PA_DATA, pa_data, sizeof(wifi_calibration_param.pa_trim_data))) {
        return 1;
    } else {
        norflash_protect_suspend();//解除flash写保护
        if (user_rf_fcc_info_write(VM_WIFI_PA_DATA, pa_data, sizeof(wifi_calibration_param.pa_trim_data), 0)) {
            norflash_protect_resume();
            return 1;
        }
        norflash_protect_resume();
