...
int wifi_set_flash_mac(char *mac);
int wifi_get_flash_mac(char *mac);
int user_rf_fcc_info_write(int index, unsigned char *data, int size, char trim);
int user_rf_fcc_info_read(int index, unsigned char *data, int size);
...
static int create_fix_cmd_data(u8 opcode, void *data, u32 len, u8 *buf)
{
    u16 crc;
    u32 offset;
    //u8 buf[160] = {0};
...
int user_mac_wr(u8 *uart_data, int len)
{
    unsigned char mac_buf[6] = {0};
    unsigned char mac_write_buf[10] = {0x01, 0xA1, 0xA2, 0x06, 0x01, 0x02, 0x03, 0x04, 0x05, 0x09};
    unsigned char mac_read_buf[10] = {0x01, 0xA1, 0xA2, 0x06, 0x01, 0x02, 0x03, 0x04, 0x05, 0x0A};

    if (len < 10) {
        return len;
    }
    if (!memcmp(mac_write_buf, uart_data, sizeof(mac_write_buf))) {
        memcpy(mac_buf, uart_data + sizeof(mac_write_buf), 6);
        len = create_fix_cmd_data(OP_FCC_USER_SET_MAC, mac_buf, 6, uart_data);
        printf("==========MAC COM=================\n");
        put_buf(uart_data, len);
    } else if (!memcmp(mac_read_buf, uart_data, sizeof(mac_read_buf))) {
        memset(mac_buf, 0, 6);
        len = create_fix_cmd_data(OP_FCC_USER_INQ_MAC, mac_buf, 6, uart_data);
        printf("==========MAC COM=================\n");
        put_buf(uart_data, len);
    }
    return len;
    ...
    for (;;) {
        list_for_each_entry(host, &head, entry) {
            ...
            case OP_FCC_SET_MAC:
                ...
                if (wifi_set_flash_mac(fcc->params) > 0) {
                    status = ST_SUCC;
                } else {
                    u8 mac_read[6] = {0};
                    wifi_get_flash_mac(mac_read);
                    if (!memcmp(fcc->params, mac_read, 6)) {
                        status = ST_SUCC;
                    } else {
                        status = ST_FAIL;
                    }
                }
                printf("GET MAC : status = %d \n", status);
                report_data(OP_FCC_RPT_ST, &status, sizeof(status));
                break;
            ...
            case OP_FCC_USER_SET_MAC:
                //wifi_set_mac(fcc->params);
                unsigned char mac_response_set_ok[] = {0x04, 0x0E, 0x01, 0x01, 0xA1, 0xA2, 0x00};
                if (wifi_set_flash_mac(fcc->params) > 0) {
                    status = ST_SUCC;
                    comm_dev_tx_data(mac_response_set_ok, sizeof(mac_response_set_ok));
                    put_buf(mac_response_set_ok, sizeof(mac_response_set_ok));
                } else {
                    u8 mac_read[6] = {0};
                    wifi_get_flash_mac(mac_read);
                    if (!memcmp(fcc->params, mac_read, 6)) {
                ...
                printf("SET MAC : status = %d \n", status);
                break;

            case OP_FCC_USER_INQ_MAC:
                memset(mac, 0, sizeof(mac));
                wifi_get_flash_mac(mac);
                //wifi_get_mac(mac);
                printf("GET MAC= %X:%X:%X:%X:%X:%X\n", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
                unsigned char mac_response_req_ok[] = {0x04, 0x0E, 0x01, 0x01, 0xA1, 0xA2, 0x00};
                unsigned char mac_response[32] = {0};
                memcpy(mac_response, mac_response_req_ok, sizeof(mac_response_req_ok));
                memcpy(mac_response + sizeof(mac_response_req_ok), mac, 6);
                comm_dev_tx_data(mac_response, sizeof(mac_response_req_ok) + 6);
                put_buf(mac_response, sizeof(mac_response_req_ok) + 6);
                break;
            ...
            case OP_IN_FREQ_ADJ:
                ...
                if (adj_cnt > max_cnt || (cur_freq >= thr_min && cur_freq <= thr_max)) {
                    ...
                    if (status == ST_SUCC) {
                        ...
                        ret = user_rf_fcc_info_write(VM_XOSC_INDEX, cur_xosc, sizeof(cur_xosc), 1);
                        if (!ret) {
            ...
            case OP_IN_PWR_ADJ:
                ...
                if (adj_cnt > max_cnt || (cur_pwr >= thr_min && cur_pwr <= thr_max)) {
                    ...
                    if (status == ST_SUCC) {
                        ...
                        printf("->wifi_set_mcs_dgain \n");
                        put_buf(gain_array, sizeof(gain_array));
                        norflash_protect_suspend();//解除flash写保护
                        ret = user_rf_fcc_info_write(VM_WIFI_PA_MCS_DGAIN, gain_array, sizeof(gain_array), 1);
                        if (!ret) {
            ...
            case OP_FCC_INQ_DEF_DATA:
                ...
                if (!user_rf_fcc_info_read(VM_WIFI_PA_DATA, (u8 *)def_data.pa, sizeof(def_data.pa))) {
                    syscfg_read(VM_WIFI_PA_DATA, (u8 *)def_data.pa, sizeof(def_data.pa));
                }
                if (!user_rf_fcc_info_read(VM_XOSC_INDEX, (u8 *)def_data.xosc, sizeof(def_data.xosc))) {
                    syscfg_read(VM_XOSC_INDEX, (u8 *)def_data.xosc, sizeof(def_data.xosc));
                }
                if (!user_rf_fcc_info_read(VM_WIFI_PA_MCS_DGAIN, def_data.mcs_dgain, sizeof(def_data.mcs_dgain))) {
...
void clear_efuse(void)
{
    ...
    norflash_protect_suspend();//解除flash写保护
    if (!user_rf_fcc_info_write(VM_XOSC_INDEX, xosc, 2, 1)) {
        syscfg_write(VM_XOSC_INDEX, xosc, 2);
    }
    if (!user_rf_fcc_info_write(VM_WIFI_PA_MCS_DGAIN, wifi_calibration_param.mcs_dgain, sizeof(wifi_calibration_param.mcs_dgain), 1)) {
...
u8 fcc_enter_io_bt_mode(void)
{
    if (CUR_MODE == FCC_WIFI_MODE || CUR_MODE == 0) {
        return 0;
    }
    return 1;
}
...
u8 rf_fcc_test_init(void)
{
    ...
    if (!fcc_enter_io_trigger()) {//IO保持拉低才能持续进入FCC测试
        return 0;
    }
#endif
...
static void read_res(void)
{
    ...
    if (!user_rf_fcc_info_read(VM_XOSC_INDEX, &cal.xosc_l, 2)) {
        syscfg_read(VM_XOSC_INDEX, &cal.xosc_l, 2);
    }
    if (!user_rf_fcc_info_read(VM_WIFI_PA_MCS_DGAIN, &cal.mcs_dgain, sizeof(cal.mcs_dgain))) {
        syscfg_read(VM_WIFI_PA_MCS_DGAIN, &cal.mcs_dgain, sizeof(cal.mcs_dgain));
    }
    if (!user_rf_fcc_info_read(VM_WIFI_PA_DATA, &cal.pa_trim_data, sizeof(cal.pa_trim_data))) {
