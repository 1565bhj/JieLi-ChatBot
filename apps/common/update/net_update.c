...
typedef struct update_file_info {
    char FileName[16];
    u32 file_size;
    u32 finish_len;
} update_file_info;

static void net_update_percent_callback(void *priv)
{
    ...
    if (percent != tmp) {
        ...
        extern void tm_1629_shown_update_percent(int percent);
        tm_1629_shown_update_percent(percent);
#endif
#ifdef CONFIG_LVGL_UI_ENABLE
        int lv_demo_system_update_percent(int percent, int err_code);
        lv_demo_system_update_percent(percent, 0);
#endif
    }
    if (percent == 100) {
        percent = 0;
...
static void timeout_key_enable_check(char clear)
{
    static u16 time_id = 0;
    if (clear && time_id) {
        sys_timer_modify(time_id, 30 * 1000);//30秒没有收到数据包继续清空则开启按键可以开关机
        return;
    }
    if (!time_id) {
        time_id = sys_timeout_add_to_task("sys_timer", NULL, key_event_enable, 30 * 1000);//30秒没有收到数据包继续清空则开启按键可以开关机
    } else {
#ifdef CONFIG_UI_PLAY_EMOJI
        play_face_emoji_loop(0);
        play_face_emoji(AI_UART_CMD_EMOJI_DATA);
#endif
#ifdef CONFIG_LVGL_UI_ENABLE
        int lv_demo_system_update_percent(int percent, int err_code);
        lv_demo_system_update_percent(0, -2);
#endif
    }
}
...
void *net_fopen(char *path, char *mode)//net_fopen支持写flash固件升级和写到SD卡，当名字字符有CONFIG_UPGRADE_OTA_FILE_NAME时是固件升级
{
    ...
    if (strstr(path, CONFIG_UPGRADE_OTA_FILE_NAME) || strstr(path, CONFIG_UPGRADE_OTA_FILE_NAME_RES)) {//名字是OTA升级名字就进行写flash升级模式
        ...
        *check = NET_UPDATE_BUF_CHECK;
        ...
        ai_uart_cmd_data_push(AI_UART_CMD_SYS_UPDATE);
#endif
#ifdef AT_UART_CMD_ENABLE
        at_uart_cmd_send(AI_UART_CMD_SYS_UPDATE);
#endif
#ifdef CONFIG_UI_PLAY_EMOJI
        play_face_emoji(AI_UART_CMD_SYS_UPDATE);
        play_face_emoji_loop(AI_UART_CMD_SYS_UPDATE);
#endif
#ifdef CONFIG_LVGL_UI_ENABLE
        int lv_demo_system_update_percent(int percent, int err_code);
        lv_demo_system_update_percent(0, 0);
#endif
        if (strstr(path, CONFIG_UPGRADE_OTA_FILE_NAME_RES)) {
            net_update->update_res = true;
        }
        if (net_update->update_res) {
            music_play_res_file("UpdateRes.mp3");
        } else {
            music_play_res_file("UpdateDo.mp3");
        }
        music_play_waite();
        key_event_disable();
        timeout_key_enable_check(1);
        set_update_process_notify_cb(net_update_percent_callback);
...
int net_fclose(void *fd, char is_socket_err)
{
    ...
    if (net_update->write_to_flash) {
        ...
        if (net_update->update_state == NET_UPDATE_STATE_OK) {//升级成功，2s后重启系统
            if (net_update->update_res) {
                music_play_res_file("UpdateSucc.mp3");
                music_play_waite();
                aisp_all_pause(0);
                struct key_event key = {0};
                key.type = KEY_EVENT_USER;
                ...
                music_play_res_file("UpdateSuRst.mp3");
                music_play_waite();
                struct key_event key = {0};
//              key.type = KEY_EVENT_USER;
//              key.action = KEY_EVENT_LONG;
//              key.value = KEY_CPU_RESET;
//              key_event_notify(KEY_EVENT_FROM_USER, &key);
...
#ifdef CONFIG_LVGL_UI_ENABLE
            int lv_demo_system_update_percent(int percent, int err_code);
            lv_demo_system_update_percent(0, 1);
#endif
        } else {
            err = -1;
            printf(">>>>>>>>>>>net_update err\n\n\n\n");
            music_play_res_file("UpdateErr.mp3");
            music_play_waite();
            ...
            while (cp_remain) {
                if (!net_update->recv_cnt) {//第一次写则初始化双备份升级
                    ...
                    if (first_size) {
                        ...
                        if (err) {
                            net_update->update_state = NET_UPDATE_STATE_ERR;
                            printf("dual_bank_update_write err \n\n");
                            return err;
                        }
                ...
                timeout_key_enable_check(1);//启动超时开启按键
                //写数据到flash
                err = dual_bank_update_write(cp_buf, cp_remain, net_update_doing_callback);
                if (err) {
                    net_update->update_state = NET_UPDATE_STATE_ERR;
                    printf("dual_bank_update_write err 2\n\n");
                    return err;
                }
