...
#define IO_INIT()   {JL_PORTC->DIR &=~(BIT(6)|BIT(7));JL_PORTC->OUT &=~(BIT(6)|BIT(7))}
#define IO1_SET(x)  if(x){JL_PORTC->OUT |= BIT(6);}else{JL_PORTC->OUT &=~BIT(6);}
#define IO2_SET(x)  if(x){JL_PORTC->OUT |= BIT(7);}else{JL_PORTC->OUT &=~BIT(7);}
