...
#define KEY_LONG_EVENT_CONTINUE_NOTIFY  1   //是否支持长按消息连续下发
#define KEY_LONG_EVENT_CONTINUE_CNT     30   //长按消息连续下发间隔扫描次数
...
static volatile u8 key_first = 0;
...
static void key_driver_scan(void *_scan_para)
{
    ...
    //过滤开机一直按按键
    if (!scan_para->first_key) {
        if (cur_key_value != NO_KEY) {
            return;
        } else {
            scan_para->first_key = cur_key_value;
        }
    }
...
//===== 按键消抖结束, 开始判断按键类型(单击, 双击, 长按, 多击, HOLD, (长按/HOLD)抬起)
    if (cur_key_value != scan_para->last_key) {
        if (cur_key_value == NO_KEY) {  //cur_key = NO_KEY; last_key = valid_key -> 按键被抬起
            ...
            /* } */
#else
#if KEY_LONG_EVENT_CONTINUE_NOTIFY
            if (scan_para->press_cnt && scan_para->last_event == KEY_EVENT_LONG) {
                key_event = KEY_EVENT_UP;
                key_value = scan_para->last_key;
                goto _notify;   //发送抬起消息
            }
        ...
        if (cur_key_value == NO_KEY) {  //last_key = NO_KEY; cur_key = NO_KEY -> 没有按键按下
            if (scan_para->click_cnt > 0) {  //有按键需要消息需要处理
                ...
                if (scan_para->click_delay_cnt > scan_para->click_delay_time) { //按键被抬起后延时到
                    ...
                    if (scan_para->click_cnt >= 7) {
                        key_event = KEY_EVENT_MORE_CLICK;  //七击以上
                    } else if (scan_para->click_cnt >= 6) {
                        key_event = KEY_EVENT_SIX_CLICK;  //六击以上
                    }*/
                    if (scan_para->click_cnt >= 6) {
                        key_event = KEY_EVENT_MORE_CLICK;  //六击以上
            ...
            scan_para->press_cnt++;
#if KEY_LONG_EVENT_CONTINUE_NOTIFY
            if (scan_para->last_event == KEY_EVENT_LONG && scan_para->press_cnt >= (KEY_LONG_EVENT_CONTINUE_CNT + 1)) {
...
_scan_end:
    ...
    if (key_event == KEY_EVENT_LONG) {
        scan_para->press_cnt = 1;//长按可以连续通知事件
        scan_para->last_event = key_event;
    } else if (key_event == KEY_EVENT_UP) {
        scan_para->last_event = 0xFF;
    }
#endif
