...
extern int flash_write_protect(void);
void __attribute__((weak)) board_init() {}
void __attribute__((weak)) board_early_init() {}

static void app_task_handler(void *p)
{
#if (!defined CONFIG_SFC_ENABLE && (defined PRODUCTION_TEST_ENABLE || defined PRODUCTION_ALL_TEST_ENABLE))
    wdt_reset_enable();//跑sdram，量产时候看们狗直接复位系统防止看们狗异常中断了卡死
#endif
    sys_timer_init();
    sys_timer_task_init();
    ...
    extern production_io_init(void);
    production_io_init();//PC5触发厂测需要再这里提前初始化（部分产品使用PC5作为功能默认拉低会导致部分功能异常）
#endif
