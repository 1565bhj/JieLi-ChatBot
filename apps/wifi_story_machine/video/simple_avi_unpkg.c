...
#define JL_AVI_FILE_MOV_ADDR    (512-4)

#define JPEG_FRAM_SIZE_DEFALUT_SIZE     (100*1024)
...
struct avi_head_str {
    ...
    u32 idx1_addr;
    float per_adframe_time;//每帧音频时间
    float per_vdframe_time;//每帧视频时间
...
static int avi_read_head(FILE *fd, u8 state)
{
    ...
    avi_info->fd = fd;
    avi_info->flag = true;
    avi_info->movi_addr = JL_AVI_FILE_MOV_ADDR;
    fseek(fd, 0, SEEK_SET);
    return 0;
}
...
int avi_get_audio_per_frame_size(u8 state)
{
    struct avi_head_str *avi_info = avi_get_head_handler(state);
    return avi_info->per_adframe_size;
}
int avi_get_audio_per_frame_time(u8 state)
{
    struct avi_head_str *avi_info = avi_get_head_handler(state);
    return avi_info->per_adframe_time;
}
int avi_get_video_per_frame_time(u8 state)
{
    struct avi_head_str *avi_info = avi_get_head_handler(state);
    return avi_info->per_vdframe_time;
}
static int avi_get_audio_perframe_size(FILE *fd, int addr, u8 state)
{
...
read_index:
    ...
    if (idex1_head.str != idx1 && avi_info->movi_addr == JL_AVI_FILE_MOV_ADDR) {
        fseek(fd, 0, SEEK_SET);
    ...
    if (!avi_info->per_adframe_time) {
        len = avi_get_audio_perframe_size(fd, addr, state);
        avi_info->per_adframe_size = len;
        if (len > 0 && avi_info->file_head.aud_strf.nAvgBytesPerSec > 0) {
            avi_info->per_adframe_time = (float)len * 1000 / avi_info->file_head.aud_strf.nAvgBytesPerSec;
            avi_info->per_vdframe_time = (float)avi_info->file_head.avih.dwMicroSecPerFrame / 1000;
            avi_info->video_num_coefficient = (float)avi_info->per_adframe_time / vd_presec_ms;
            avi_info->audio_num_coefficient = (float)avi_info->per_vdframe_time / ad_presec_ms;
            //printf("ad_presec_ms : %f , %f , %f \n",ad_presec_ms,vd_presec_ms,avi_info->video_num_coefficient);
            //printf("len  %d, dwRate : %d, %d\n",len,avi_info->file_head.aud_strh.dwRate, avi_info->file_head.aud_strf.nBlockAlign);
            //printf("ad_presec_ms : %f\n",avi_info->per_adframe_time);
            //printf("vd_presec_ms : %f\n",avi_info->per_vdframe_time);
        } else {
            printf("warning AVI NO audio frame\n");
            avi_info->per_adframe_time = ((float)8184 / (8000 * 2)) * 1000; //ms
        }
    }
    if (!avi_info->per_vdframe_time) {
        avi_info->per_vdframe_time = (float)avi_info->file_head.avih.dwMicroSecPerFrame / 1000;
    }
    //5 . 返回第一个索引表地址
...
//    printf("--->idx_offset = 0x%x ,index_info.dwOffset = 0x%x, index_info.dwSize = %d\n",idx_offset,index_info.dwOffset,index_info.dwSize);
    all_len = fread(buf, 1, index_info.dwSize, fd);
...
#if JPEG_DEC_MUC_FIX
#define JPEG_WIDTH      240 //分辨率宽,用户根据实际应用更改，和解码图片分辨率相关！！！！
...
int avi_unpkg_decoder_to_yuv_example(int (*lcd_write_callback)(char *buf, int size))
{
    int ret;
    char name[64];
    char *fbuf = NULL;
    ...
    while (1) {
        ...
        if (ret > 0) {
            ...
            if (jpeg_decode_image_info(&info)) {//获取JPEG图片信息
                ...
                if (lcd_write_callback) {
                    if (lcd_write_callback(rgb565, info.width, info.height * 2)) {
                        printf("lcd_write_callback err\n");
                        break;
                    }
                }
