    ...
    do {
        if (!mfs->p_addr) {
            ...
            if (check_a2dp_media_if_mute()) {
                ...
                if (++log_count % 600 == 0) {  // 每100次打印一次日志
                    log_debug("有蓝牙音频");
                    lv_demo_music_play_pause_button(0);//播放图标显示
                }
#endif
            }
        }
...
void *earphone_a2dp_audio_codec_open(int media_type, u8 volume)
{
    ...
    if (!os_mutex_valid(&mfs->mutex)) {
        os_mutex_create(&mfs->mutex);
    }
#endif
    ...
    //printf("-> sbc volume = %d \n",volume);
    volume += 30;//蓝牙播放音乐有点小声
    req.dec.cmd             = AUDIO_DEC_OPEN;
    req.dec.volume          = GET_SET_VOLUME(volume ? volume : get_app_music_volume());
    ...
    req.dec.effect = AUDIO_EFFECT_SPECTRUM_FFT;
    server_request(mfs->audio_dec, AUDIO_REQ_DEC, &req);
    mfs->spectrum_fft_hdl = req.dec.get_hdl;
#endif
...
#ifdef CONFIG_SPECTRUM_FFT_EFFECT_ENABLE
    os_mutex_pend(&mfs->mutex, 6000);
    mfs->spectrum_fft_hdl = NULL;
#endif
    mfs->a2dp_open = 0;
...
int bt_music_decode_stop_status(void)
{
    return mfs->audio_dec ? 0 : 1;
}
short *bt_music_get_audio_fft(int *len)
{
#ifdef CONFIG_SPECTRUM_FFT_EFFECT_ENABLE
    extern int bt_music_play_get_pause(void);
    if (!os_mutex_valid(&mfs->mutex) || !mfs->spectrum_fft_hdl || bt_music_play_get_pause()) {
        return NULL;
    }
    os_mutex_pend(&mfs->mutex, 6000);
    if (!mfs->spectrum_fft_hdl || bt_music_play_get_pause()) {
        os_mutex_post(&mfs->mutex);
        return NULL;
    }
    if (mfs->spectrum_fft_hdl) {
        short *db_data = audio_spectrum_fft_get_val(mfs->spectrum_fft_hdl);//获取存储频谱值得地址
        if (db_data) {
            int n = audio_spectrum_fft_get_num(mfs->spectrum_fft_hdl);
            *len = n;
            os_mutex_post(&mfs->mutex);
            return db_data;
//                put_buf(db_data, n * 2);
//                for (int i = 0; i < audio_spectrum_fft_get_num(mfs->spectrum_fft_hdl); i++) {
//                    //输出db_num个 db值
