...
#define AP_SSID "AC79_WiFi_"      //配置 AP模式的SSID前缀
#define AP_PWD  "88888888"                //配置 AP模式的密码
...
//    wifi_set_airkiss_key(airkiss_aes_key);        //配置 airkiss_aes_key
        ...
        //strncpy(ssid, AP_SSID, sizeof(ssid) - 1);
...
int wifi_ap_mode_init(void)
{

    wl_wifi_cur_mode_set(WIFI_MODE_AP);
    return os_task_create(wifi_demo_task, NULL, 10, 1000, 0, "wifi_demo_task");
}

int wifi_ap_mode_deinit(void)
{
    wl_wifi_cur_mode_set(WIFI_MODE_NONE);
    return 0;
}
