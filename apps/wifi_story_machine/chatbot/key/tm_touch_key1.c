...
#ifdef TCFG_TOUCH_KEY_PORT1
#if (TCFG_TOUCH_KEY_PORT1 != -1)
...
static void tm_touch_key_scan1(void)
{
    int io_sta = gpio_read(TCFG_TOUCH_KEY_PORT1);
    if (!io_sta) {
        touch_key_enter++;
...
//        if(touch_key_enter >= (10000 / TOUCH_KEY_SCAN_TIME) && !touch_reset_cfg){
//            touch_reset_cfg = 1;
//            system_restore_factory_settings();
//            sys_timeout_add(NULL, system_reset, 500);
//        }
...
static void tm_touch_key_scan_init1(void)
{
    gpio_direction_input(TCFG_TOUCH_KEY_PORT1);
    gpio_set_pull_down(TCFG_TOUCH_KEY_PORT1, 0);
    gpio_set_pull_up(TCFG_TOUCH_KEY_PORT1, 1);
    sys_timer_add_to_task("sys_timer", NULL, tm_touch_key_scan1, TOUCH_KEY_SCAN_TIME);
}
static void tm_touch_key_init1(void)
{
    if (production_test_io_get()) {
        return;
    }
    sys_timeout_add_to_task("sys_timer", NULL, tm_touch_key_scan_init1, 1000);//1秒后再检测按键
}
late_initcall(tm_touch_key_init1);
#endif
#endif
