...
#define SD_MUSIC_FILE_PLAY_EN           1  //SD卡音乐按文件夹播放使能

#if SD_MUSIC_FILE_PLAY_EN
#define SD_STORY_STEP_FILE_PLAY_EN      1   //SD卡跳文件夹使能
#define SD_STORY_FILE_PLAY_RAND_EN      0   //SD卡音乐随机播放
#define SD_STORY_FILE_PLAY_STEP_NUM     10  //SD卡音乐下十个故事
#endif

// 播放暂停控制函数
int sd_music_play_pause(char pause);
int sd_music_play_prev(void);  // 新增上一曲函数声明
int sd_music_play_next(void);  // 新增下一曲函数声明
int sd_music_set_volume(int volume);  // 新增直接设置音量值函数声明
...
extern void sdx_dev_detect_timer_del(void);
...
//设置音量大小
int sd_music_set_dec_volume(int volume)
{
    union audio_req req = {0};
...
int sd_music_play_next(void)// 下一曲播放
{
...
int sd_music_set_volume(int volume)// 直接设置音量值
{
    // 调用已有的sd_music_set_dec_volume函数来设置音量
    return sd_music_set_dec_volume(volume);
}
...
int sd_music_play_loop_set(char enable)
{
    __this->play_loop = enable;
    return __this->play_loop;
}
int sd_music_stop_status(void)
{
...
int sd_music_pause_status(void)
{
    if (__this->puase) {
        return 1;
    }
...
{
#if (SD_STORY_FILE_PLAY_RAND_EN == 0)
    __this->last_rand = 0;
#endif
    int next = 0;
    if (next_dir) {
        next = next_dir > 0 ? next_dir : -next_dir;
        if (next == SD_STORY_FILE_PLAY_STEP_NUM) {
...
#endif // SD_MUSIC_FILE_PLAY_EN

int user_sd_music_dir_file_get(int *dir, int *filen)
{
    if (dir) {
...
static void sd_music_dir_file_num_write_to_flash(void)
{
    if (__this->last_dir_num && __this->last_file_num) {
...
#if SD_STORY_STEP_FILE_PLAY_EN
        if (key->value == KEY_DOWN) { //V+
            local_music_play_dir_add(SD_STORY_FILE_PLAY_STEP_NUM);//文件夹
        } else if (key->value == KEY_UP) { //V-
            local_music_play_dir_add(-SD_STORY_FILE_PLAY_STEP_NUM);//文件夹
        } else if (key->value == KEY_MODE) { //mode
            local_music_play_file_add(SD_STORY_FILE_PLAY_STEP_NUM);//文件
        }
#ifdef  USED_TM1629_SHOWN
        if (key->value == KEY_DOWN || key->value == KEY_UP) { //V+ V-
            tm_1629_shown_set_dir_file_num(__this->last_dir_num, 0);
...
static int local_music_device_event_handler(struct sys_event *sys_eve)
{
    ...
    if (sys_eve->from == DEVICE_EVENT_FROM_SD) {
        ...
        case DEVICE_EVENT_IN://SD卡插入
            ...
            break;
        case DEVICE_EVENT_OUT://SD卡拔出，释放资源
            puts("->SD OUT\n");
            __this->event_out = true;
            sd_music_dir_file_num_write_to_flash();
            local_music_switch_local_device(NULL);
#if (defined CONFIG_SD_AUTO_ENABLE)
            extern int audio_app_mode_switch_set(char *name, char note);
