...
typedef int (*aiui_http_recv_cb)(const void *param, AIUI_HTTP_EVENT_TYPE type, void *buf, const int len);
/*
    创建http请求句柄:
        url:   请求的url
        type:  请求的类型(GET/POST)
        cb:    数据接收回调函数，与aiui_http_recv函数互斥，即设置了回调函数就不能再用aiui_http_recv接收数据了.
        param: 回调函数的参数,
        name: 接收线程的名称,不设置回调函数传空即可(http创建接收线程时，需要线程名称)
    return: 返回一个句柄，返回NULL，则创建失败
*/
AIUI_HTTP_HANDLE aiui_http_create_request(const char *url, const char *type,
        const aiui_http_recv_cb cb, void *param,
        const char *name);

/*
    添加http头文件信息
        handle: aiui_http_create_request返回值
        data:头信息内容,必须带'\0'
*/
...
/*
    接收主机端发送的数据: (目的是方便客户端读取数据量较小的http连接,比如音乐授权、获取音乐url等)
...
/*
    销毁http请求
        handle: aiui_http_create_request返回值
*/
void aiui_http_destory_request(AIUI_HTTP_HANDLE handle);
#endif //
